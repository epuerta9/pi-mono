...
### 8.4 The Broker Is an Implementation Detail

NATS is the *recommended* transport, **not a hard dependency**. Everything in §8
talks to a `Transport` interface (`Publish` / `Request` / `Subscribe`); the broker
behind it is swappable. This keeps the mesh honest and de-risks licensing.

| Transport | Role | When |
|-----------|------|------|
| **InProcess** (Go channels) | default; no broker at all | solo mode, local extensions — the 80% |
| **Embedded NATS** | pub/sub + request/reply + JetStream KV/persistence + clustering | mesh / remote / multi-agent |
| **gRPC streams** | point-to-point RPC + streaming only | calling one known service; **not** a broker (no fan-out, discovery, or persistence) |

**Tiered extension model** (the broker only appears at the heavy end):

1. **In-process Go** — providers, core UI, the loop.
2. **Starlark + rich SDK** — local glue, commands, interception, and the *routing
   brain* that decides which hook goes to which subject. Sandboxed; the safe target
   for **agent-authored, hot-reloaded** extensions.
3. **WASM** — sandboxed real-language compute (capability-gated HTTP via host
   functions / `wasi-http`).
4. **NATS services** — persistent, remote, any-language; supervised (see below).

An extension keeps the **same contract** across tiers, so it can *graduate* from
in-process Starlark to a supervised NATS service with zero API change.

> **Licensing note.** NATS core is **Apache 2.0 under the CNCF**. After the
> Apr–May 2025 Synadia/CNCF dispute (an attempt to move future releases to BSL and
> exit CNCF), stewardship and the Apache-2.0 line stayed with CNCF — **no
> commercial-use issue today**. The `Transport` abstraction is the hedge against any
> future divergence.

### 8.5 Supervision (Keeping Extension Services Alive)

NATS is a **broker, not a process manager**. Long-lived extension/agent processes
are kept alive by an Erlang-style **supervisor tree** (`github.com/thejerf/suture/v4`:
context-aware, backoff, failure-rate thresholds, composable). Liveness is tracked
two ways: OS process exit **and** a NATS heartbeat (`mesh.health.{name}`); missed
beats ⇒ kill + restart with backoff (OTP one-for-one). Subject-level **NATS
accounts/permissions** give per-extension capability isolation.

> **Self-extension, done safely.** Pi has *no* first-class self-extension API and
> runs extensions with full system permissions (its docs warn "only install from
> sources you trust"). Here, an agent emits a sandboxed **Starlark** extension that
> hot-reloads — first-class self-modification *without* arbitrary-code risk. This is
> the capability isolation pi structurally lacks.
